IF EXISTS (SELECT * FROM sys.triggers WHERE name = 'tr_FA_TRAN_DOCUMENTOS25082011')
...
BEGIN
		IF(SELECT COUNT(1) FROM temp_registroMigracion where nombre_table = 'FA_TRAN_DOCUMENTOS25082011' AND tipo=@tipo AND codigo=@codigo AND codigo2=@codigo2 AND codigo3=@codigo3 AND codigo4=@codigo4) = 0
...
IF EXISTS (SELECT * FROM sys.triggers WHERE name = 'TR_FA_TRAN_DOCUMENTOS25082011_UP')
