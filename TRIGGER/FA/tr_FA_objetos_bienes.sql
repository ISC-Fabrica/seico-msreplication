IF EXISTS (SELECT * FROM sys.triggers WHERE name = 'tr_FA_objetos_bienes')
...
BEGIN
			IF(SELECT COUNT(1) FROM temp_registroMigracion where nombre_table = 'FA_objetos_bienes' AND tipo=@tipo AND codigo=@codigo AND codigo2=@codigo2 AND codigo3=@codigo3 AND codigo4=@codigo4) = 0
...
IF EXISTS (SELECT * FROM sys.triggers WHERE name = 'TR_FA_objetos_bienes_UP')
