IF EXISTS (SELECT * FROM sys.triggers WHERE name = 'tr_FA_TEMP_CXCOBRAR_EXTRA')
...
BEGIN
			IF(SELECT COUNT(1) FROM temp_registroMigracion where nombre_table = 'FA_TEMP_CXCOBRAR_EXTRA' AND tipo=@tipo AND codigo=@codigo AND codigo2=@codigo2 AND codigo3=@codigo3 AND codigo4=@codigo4) = 0
...
IF EXISTS (SELECT * FROM sys.triggers WHERE name = 'TR_FA_TEMP_CXCOBRAR_EXTRA_UP')
