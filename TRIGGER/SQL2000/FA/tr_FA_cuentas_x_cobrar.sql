IF EXISTS (SELECT * FROM sysobjects WHERE type = 'TR' and name = 'tr_FA_cuentas_x_cobrar')
...
IF @tipo IS NOT NULL AND @codigo !='' AND
		   NOT EXISTS (SELECT 1 FROM temp_registroMigrado WHERE nombre_table = 'FA_cuentas_x_cobrar'
...
IF EXISTS (SELECT * FROM sysobjects WHERE type = 'TR' and name = 'TR_FA_cuentas_x_cobrar_UP')
