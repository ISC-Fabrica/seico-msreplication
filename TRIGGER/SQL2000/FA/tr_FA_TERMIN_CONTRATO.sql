IF EXISTS (SELECT * FROM sysobjects WHERE type = 'TR' and name = 'tr_FA_TERMIN_CONTRATO')
...
IF @tipo IS NOT NULL AND @codigo !='' AND
		   NOT EXISTS (SELECT 1 FROM temp_registroMigrado WHERE nombre_table = 'FA_TERMIN_CONTRATO'
...
IF EXISTS (SELECT * FROM sysobjects WHERE type = 'TR' and name = 'TR_FA_TERMIN_CONTRATO_UP')
